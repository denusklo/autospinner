...
**Standard regression command (the concrete form of R2)**: run `node verify.js` in the repo root — 74 checks (simulator unit tests + DoraSolver solution legality + old/new A/B + sealed columns/CELL_FLAGS constraints + minFirstCombos steering + TargetPlanner + exact-N + router v2 + priority cells + solveMaxFirstCombos + start/end pinning + smoke regression); exit 0 = all pass. Must run after changing algorithm.js; when adding behavior, add a matching check to verify.js. [Verification] actual run 74/74 PASS, 2026-07-08
