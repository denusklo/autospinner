...
| DG-06 | Model routes become unavailable or drift | Agent launch reports unknown model/effort; TOML/docs/validator disagree | User-approved explicit root/agent pins plus native/shared assertions | On Codex/model update; quarterly | Verify installed availability, propose a K3 remap, change one route at a time, validate, then test representative dispatch | Commander/User for authority changes | TOML valid; exact routing assertions and representative role dispatch pass |
...
- Installed Codex `0.144.1` exposed Sol, Terra, and Luna with the approved efforts before the 2026-07-13 migration; future availability and actual project-role loading still require a new trusted-session check.
